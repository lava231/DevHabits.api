...
public sealed class UpdateHabitDtoValidator : AbstractValidator<UpdateHabitDto>
{
    ...
    public UpdateHabitDtoValidator()
    {
        RuleFor(x => x.Name)
            ...
            .MinimumLength(3)
            .MaximumLength(15)
            .WithMessage("Habit name must be between 3 and 15 characters");
        ...
        RuleFor(x => x.Target.Unit)
            .Must((dto, unit) =>
                IsTargetUnitCompatibleWithType(dto.Type, unit))
            .WithMessage("Target unit is not compatible with the habit type");
    }

    private static bool IsTargetUnitCompatibleWithType(
        HabitType type,
        string unit)
    {
        ...
        {
            HabitType.Binary =>
                AllowedUnitsForBinaryHabits.Contains(normalizedUnit),

            HabitType.Measurable =>
                AllowedUnits.Contains(normalizedUnit),
